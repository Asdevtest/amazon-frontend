...
import {useClassNames} from './product-search-request-form.style'
...
export const ProductSearchRequestForm = ({onSubmit, setOpenModal, isEdit, requestToEdit}) => {
  ...
  const sourceFormFields = {
    ...
    denyProductsBoughtByMe: requestToEdit?.denyProductsBoughtByMe || false,
    denyProductsExistInServiceByCurStrategy: requestToEdit?.denyProductsExistInServiceByCurStrategy || false,
    findSupplier: requestToEdit?.findSupplier || false,
    checkedByService: requestToEdit?.checkedByService || false,
  }
  ...
  const onChangeField = fieldName => event => {
    ...
    if (numberFields.includes(fieldName) && !checkIsPositiveNummberAndNoMoreTwoCharactersAfterDot(event.target.value)) {
      ...
      [
        'denyProductsBoughtByMe',
        'denyProductsExistInServiceByCurStrategy',
        'findSupplier',
        'checkedByService',
      ].includes(fieldName)
    ) {
    ...
    <div className={classNames.root}>
      <div className={classNames.form}>
        ...
        <div className={classNames.checkboxWrapper}>
          <Checkbox
            color="primary"
            checked={formFields.denyProductsBoughtByMe}
            onChange={onChangeField('denyProductsBoughtByMe')}
          />
          <Typography>{textConsts.formCheckboxNoPurchasedProducts}</Typography>
        </div>

        <div className={classNames.checkboxWrapper}>
          <Checkbox
            color="primary"
            checked={formFields.denyProductsExistInServiceByCurStrategy}
            onChange={onChangeField('denyProductsExistInServiceByCurStrategy')}
          />
          <Typography>{textConsts.formCheckboxForbidLabel}</Typography>
        </div>

        <div className={classNames.checkboxWrapper}>
          <Checkbox color="primary" checked={formFields.findSupplier} onChange={onChangeField('findSupplier')} />
        ...
        <div className={classNames.checkboxWrapper}>
          <Checkbox
            color="primary"
            checked={formFields.checkedByService}
            onChange={onChangeField('checkedByService')}
          />
