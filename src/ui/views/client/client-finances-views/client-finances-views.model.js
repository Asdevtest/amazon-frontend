...
import {clientFinancesViewColumns} from '@components/table-columns/client/client-finances-columns'

import {clientFinancesDataConverter} from '@utils/data-grid-data-converters'
...
export class ClientFinancesViewsModel {
  ...
  activeSubCategory = SettingsModel.activeSubCategoryState[ActiveSubCategoryTablesKeys.CLIENT_FINANCES] || 0
  drawerOpen = false
  ...
  columnsModel = clientFinancesViewColumns()
  ...
  setActiveSubCategoryState(state) {
    SettingsModel.setActiveSubCategoryState(state, ActiveSubCategoryTablesKeys.CLIENT_FINANCES)
  }
  ...
  getDataGridState() {
    ...
    if (state) {
      ...
      this.columnsModel = clientFinancesViewColumns().map(el => ({
        ...el,
  ...
  async getPayments(activeSubCategory) {
    try {
      ...
      runInAction(() => {
        this.currentFinancesData = clientFinancesDataConverter(result)
