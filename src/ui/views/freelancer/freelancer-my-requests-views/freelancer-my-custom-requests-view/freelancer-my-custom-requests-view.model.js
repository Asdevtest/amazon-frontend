...
export class FreelancerMyCustomRequestsViewModel {
  history = undefined
  ...
  setDataGridState(state) {
    ...
    SettingsModel.setDataGridState(requestState, DataGridTablesKeys.FREELANCER_CUSTOM_MY_REQUESTS)
  }

  getDataGridState() {
    const state = SettingsModel.dataGridState[DataGridTablesKeys.FREELANCER_CUSTOM_MY_REQUESTS]
  ...
  async getMyCustomRequests() {
    try {
      ...
      runInAction(() => {
        this.searchRequests = freelancerCustomRequestsDataConverter(result)
      })
    } catch (error) {
      console.log(error)
    }
  ...
  onClickTableRow(item) {
    ...
    this.history.push('/freelancer/custom-search-request', {request: toJS(this.tmpSelectedRow.originalData)})
