...
export class FreelancerVacantProductsRequestsViewModel {
  history = undefined
  ...
  setDataGridState(state) {
    ...
    SettingsModel.setDataGridState(requestState, DataGridTablesKeys.FREELANCER_PRODUCT_VACANT_REQUESTS)
  }

  getDataGridState() {
    const state = SettingsModel.dataGridState[DataGridTablesKeys.FREELANCER_PRODUCT_VACANT_REQUESTS]
  ...
  async getRequestsVacant() {
    try {
      const result = await RequestModel.getRequests(RequestType.SEARCH_PRODUCT, RequestSubType.VACANT)
      runInAction(() => {
