...
export class FreelancerVacantNichesRequestsViewModel {
  history = undefined
  ...
  columnsModel = freelancerProductsRequestsViewColumns(typeOfRequests.NICHE)
  ...
  setDataGridState(state) {
    ...
    SettingsModel.setDataGridState(requestState, DataGridTablesKeys.FREELANCER_NICHE_VACANT_REQUESTS)
  }

  getDataGridState() {
    const state = SettingsModel.dataGridState[DataGridTablesKeys.FREELANCER_NICHE_VACANT_REQUESTS]
    ...
    if (state) {
      ...
      this.columnsModel = freelancerProductsRequestsViewColumns(typeOfRequests.NICHE).map(el => ({
        ...el,
  ...
  pushToRequestContent() {
    ...
    this.history.push('/freelancer/niche-search-request', {request: toJS(requestItem)})
  }
