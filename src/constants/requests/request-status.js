...
// Статусы для проверки заявки у супервайзера (пока вроде не нужно, но статусы можно создать):
// READY_TO_VERIFY_BY_SUPERVISOR - клиент отправляет заявку на проверку спервизеру, в этом статусе заявка не опубликована
// на бирже и подавать предложения нельзя, изменять заявку так же нельзя. Заявки с таким статусом доступны всем супервайзерам.
// (пока этот функционал вроде не нужен)
// VERIFYING_BY_SUPERVISOR - в процессе проверки заявки супервайзером, в этом статусе заявка не опубликована на бирже и
// подавать предложения нельзя, изменять заявку так же нельзя (пока этот функционал вроде не нужен)
// TO_CORRECT_BY_SUPERVISOR - статус выставляет супервайзер после проверки заявки, после этого статуса можно выставить только
// статус READY_TO_VERIFY и эта заявка должна попасть обратно на проверку ТОМУ ЖЕ супервайзеру что и проверял ее ранее.
// (поле supervisorId). Если супервайзер проверил все и все ок, то он выставляет статус PUBLISHED. (опять же пока можно заложить
