/**
 * Test swagger
 * testing the fastify swagger api
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The ApiV1AdminsProductsVacCheckedby model module.
 * @module model/ApiV1AdminsProductsVacCheckedby
 * @version 0.1.0
 */
class ApiV1AdminsProductsVacCheckedby {
    /**
     * Constructs a new <code>ApiV1AdminsProductsVacCheckedby</code>.
     * @alias module:model/ApiV1AdminsProductsVacCheckedby
     * @param _id {String} GUID пользователя в БД.
     * @param name {String} Имя пользователя.
     * @param email {String} email
     * @param role {Number} Код роли присвоенный пользователю.    roles.root = 0    roles.director = 1    roles.super = 2    roles.researcher = 3    roles.buyer = 4    roles.candidate = 5    
     * @param fba {Boolean} Флаг fba.
     * @param active {Boolean} Если истина - пользователь активен. Если нет - заблокирован админом.
     * @param rate {Number} Ставка, по который оплачивается сотрудник.
     */
    constructor(_id, name, email, role, fba, active, rate) { 
        
        ApiV1AdminsProductsVacCheckedby.initialize(this, _id, name, email, role, fba, active, rate);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, _id, name, email, role, fba, active, rate) { 
        obj['_id'] = _id;
        obj['name'] = name;
        obj['email'] = email;
        obj['role'] = role;
        obj['fba'] = fba;
        obj['active'] = active;
        obj['rate'] = rate;
    }

    /**
     * Constructs a <code>ApiV1AdminsProductsVacCheckedby</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ApiV1AdminsProductsVacCheckedby} obj Optional instance to populate.
     * @return {module:model/ApiV1AdminsProductsVacCheckedby} The populated <code>ApiV1AdminsProductsVacCheckedby</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ApiV1AdminsProductsVacCheckedby();

            if (data.hasOwnProperty('_id')) {
                obj['_id'] = ApiClient.convertToType(data['_id'], 'String');
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
            if (data.hasOwnProperty('email')) {
                obj['email'] = ApiClient.convertToType(data['email'], 'String');
            }
            if (data.hasOwnProperty('role')) {
                obj['role'] = ApiClient.convertToType(data['role'], 'Number');
            }
            if (data.hasOwnProperty('fba')) {
                obj['fba'] = ApiClient.convertToType(data['fba'], 'Boolean');
            }
            if (data.hasOwnProperty('active')) {
                obj['active'] = ApiClient.convertToType(data['active'], 'Boolean');
            }
            if (data.hasOwnProperty('rate')) {
                obj['rate'] = ApiClient.convertToType(data['rate'], 'Number');
            }
            if (data.hasOwnProperty('balance')) {
                obj['balance'] = ApiClient.convertToType(data['balance'], 'Number');
            }
            if (data.hasOwnProperty('balanceFreeze')) {
                obj['balanceFreeze'] = ApiClient.convertToType(data['balanceFreeze'], 'Number');
            }
            if (data.hasOwnProperty('overdraft')) {
                obj['overdraft'] = ApiClient.convertToType(data['overdraft'], 'Number');
            }
            if (data.hasOwnProperty('permissions')) {
                obj['permissions'] = ApiClient.convertToType(data['permissions'], ['String']);
            }
            if (data.hasOwnProperty('permissionGroups')) {
                obj['permissionGroups'] = ApiClient.convertToType(data['permissionGroups'], ['String']);
            }
            if (data.hasOwnProperty('masterUser')) {
                obj['masterUser'] = ApiClient.convertToType(data['masterUser'], 'String');
            }
            if (data.hasOwnProperty('allowedRoles')) {
                obj['allowedRoles'] = ApiClient.convertToType(data['allowedRoles'], ['Number']);
            }
        }
        return obj;
    }


}

/**
 * GUID пользователя в БД.
 * @member {String} _id
 */
ApiV1AdminsProductsVacCheckedby.prototype['_id'] = undefined;

/**
 * Имя пользователя.
 * @member {String} name
 */
ApiV1AdminsProductsVacCheckedby.prototype['name'] = undefined;

/**
 * email
 * @member {String} email
 */
ApiV1AdminsProductsVacCheckedby.prototype['email'] = undefined;

/**
 * Код роли присвоенный пользователю.    roles.root = 0    roles.director = 1    roles.super = 2    roles.researcher = 3    roles.buyer = 4    roles.candidate = 5    
 * @member {Number} role
 */
ApiV1AdminsProductsVacCheckedby.prototype['role'] = undefined;

/**
 * Флаг fba.
 * @member {Boolean} fba
 */
ApiV1AdminsProductsVacCheckedby.prototype['fba'] = undefined;

/**
 * Если истина - пользователь активен. Если нет - заблокирован админом.
 * @member {Boolean} active
 */
ApiV1AdminsProductsVacCheckedby.prototype['active'] = undefined;

/**
 * Ставка, по который оплачивается сотрудник.
 * @member {Number} rate
 */
ApiV1AdminsProductsVacCheckedby.prototype['rate'] = undefined;

/**
 * Баланс пользователя
 * @member {Number} balance
 */
ApiV1AdminsProductsVacCheckedby.prototype['balance'] = undefined;

/**
 * Замороженная при оплате ордера сумма.
 * @member {Number} balanceFreeze
 */
ApiV1AdminsProductsVacCheckedby.prototype['balanceFreeze'] = undefined;

/**
 * Овердрафт пользователя.
 * @member {Number} overdraft
 */
ApiV1AdminsProductsVacCheckedby.prototype['overdraft'] = undefined;

/**
 * Массив permission-ов.
 * @member {Array.<String>} permissions
 */
ApiV1AdminsProductsVacCheckedby.prototype['permissions'] = undefined;

/**
 * Массив групп permission-ов.
 * @member {Array.<String>} permissionGroups
 */
ApiV1AdminsProductsVacCheckedby.prototype['permissionGroups'] = undefined;

/**
 * GUID мастер пользователя к которму относится данный субпользователь.
 * @member {String} masterUser
 */
ApiV1AdminsProductsVacCheckedby.prototype['masterUser'] = undefined;

/**
 * Массив массив ролей.
 * @member {Array.<Number>} allowedRoles
 */
ApiV1AdminsProductsVacCheckedby.prototype['allowedRoles'] = undefined;






export default ApiV1AdminsProductsVacCheckedby;

