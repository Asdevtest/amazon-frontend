...
export const clientInventoryColumns = ({
  ...
  if (storekeepers?.length) {
    ...
    for (const storekeeper of storekeepers) {
      ...
      const storekeeperCell = {
        ...
        renderCell: ({ row }) => (
          <InStockCell
            isHideStorekeeper
            boxAmounts={row?.boxAmounts?.filter(box => box?.storekeeper?._id === storekeeper?._id)}
            boxId={row?._id}
            onClickInStock={otherHandlers.onClickInStock}
          />
        ),
        valueGetter: ({ row }) => {
          const boxAmounts = row?.boxAmounts?.filter(box => box?.storekeeper?._id === storekeeper?._id)?.[0]
            ?.amountInBoxes
      ...
      const purchaseQuantityCell = {
        ...
        renderCell: ({ row }) => {
          const currentBoxAmounts = row?.boxAmounts?.filter(box => box?.storekeeper?._id === storekeeper?._id)?.[0]
          ...
          return (
            <FourMonthesStockCell
              ...
              onClick={(storekeeperId, recommendedValue) =>
                fourMonthesStockHandlers.editRecommendationForStockByGuid(row?._id, storekeeperId, recommendedValue)
              }
            />
          )
        },
    ...
    columns.splice(11, 1, ...storekeeperCells)
  }
